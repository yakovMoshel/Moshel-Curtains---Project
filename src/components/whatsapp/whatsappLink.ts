...
/** Converts a local Israeli phone number to international digits (e.g. "0526286837" -> "972526286837"). */
export function toInternationalIsraeliPhone(localPhone: string): string {
...
export function buildWhatsAppUrl(localPhone: string, message?: string): string {
  const base = `https://wa.me/${toInternationalIsraeliPhone(localPhone)}`;
